...
def test_source_marker_is_warning_not_zero(sample_workbook):
    workbook = load_workbook(sample_workbook)
    workbook.active["E7"] = "-"
    workbook.save(sample_workbook)
    result = run_pipeline(sample_workbook)
    row = result.data[result.data["cell_address"] == "E7"].iloc[0]
    assert row["value_kind"] == "source_marker"
    assert any(issue.code == "SOURCE_MARKER" for issue in result.report.warnings)
