...
- Hover chỉ hiển thị các dòng metric, không có dòng mô tả và không thụt lề. Mỗi metric giữ ký hiệu giống legend (`■` xanh cho Tổng số/Cảnh báo, `■` đỏ cho Báo sai/Lỗi, `━●━` cam cho % báo sai); trường hợp không nhất quán chỉ gắn `⚠` cạnh giá trị, còn mô tả đầy đủ nằm trong Audit Table.
