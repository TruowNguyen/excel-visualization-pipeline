...
function renderImport(body: HTMLDivElement): void {
  ...
  body.innerHTML = `${importResult ? importOutcomeCard(importResult) : ''}
    <div id="import-live-status" class="import-live-status" role="status" aria-live="polite" aria-atomic="true">${importPhase === 'previewing' ? 'Đang đọc workbook và kiểm tra dữ liệu…' : importPhase === 'committing' ? 'Đang ghi dữ liệu và lưu phiên bản… Không đóng tab.' : ''}</div>
